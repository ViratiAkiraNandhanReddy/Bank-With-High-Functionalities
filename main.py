...
def PRODUCT_ACTIVATION() -> None:
    ...
    def is_Product_Key_Valid() -> None:
        ...
        if product_key_to_be_verified.get() in PRODUCTKEYS:
            ...
            ACTIVATEBUTTON.configure(
                fg_color="#A9C5E8", state="disabled", text_color_disabled="Black"
            )
    ...
    WINDOW.protocol(
        "WM_DELETE_WINDOW",
        lambda: WINDOW.destroy() if CONFIGURATION_JSON.get("isActivated") else exit(),
    )
    customtkinter.CTkLabel(
        WINDOW,
        text="ENTER A VALID PRODUCT KEY",
        font=("Arial", 28),
        text_color="#378F9C",
        justify="left",
    ).place(x=20, y=17)
    ...
    customtkinter.CTkLabel(
        WINDOW, text="PRODUCT KEY", font=("Roboto", 16), justify="left"
    ).place(x=20, y=170)
    product_key_to_be_verified = customtkinter.CTkEntry(
        WINDOW,
        placeholder_text="XXXX-XXXX-XXXX-XXXX-XXXX-XXXX-XXXX-XXXX-XXXX-XXXX",
        font=("Consolas", 14),
        width=407,
    )
    product_key_to_be_verified.place(x=20, y=201)
    customtkinter.CTkButton(
        WINDOW,
        text="CLICK HERE TO VISIT REPOSITORY",
    ...
    ACTIVATEBUTTON = customtkinter.CTkButton(
        WINDOW,
        text="ACTIVATE",
        text_color="Black",
        corner_radius=4,
        ...
        fg_color="#007ACC",
        hover_color="#3399FF",
        command=is_Product_Key_Valid,
    )
    ACTIVATEBUTTON.place(x=156, y=290)
    customtkinter.CTkButton(
        WINDOW,
        text="Cancel",
        corner_radius=4,
    ...
    CONTINUE = customtkinter.CTkButton(
        WINDOW,
        text="Continue",
        corner_radius=4,
        fg_color="#B0B0B0",
